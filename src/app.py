...
@hops.component(
    ...
    inputs=[
        ...
        hs.HopsString("Number", "number", "num column in Notion",hs.HopsParamAccess.LIST),
        hs.HopsString("Tag", "tag", "tag column in Notion",hs.HopsParamAccess.LIST),
...
# Post Function takes the inputs from Grasshopper and posts it to a Notion Database

def post_list_to_notion(name, color, num, tag, trigger):
    """
    ...
    # Notion API details
    NOTION_TOKEN = "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"
    DATABASE_ID = "123e89ff28f28006aa53f98e854fa2d6"
    ...
    for item in range(len(data_list)):
        ...
        row_data = {
            ...
            "properties": {
                    ...
                    "rich_text": [
                        {
                         "type": "equation",
                            "equation": {
                                "expression": f"\\color{{#000}}\\colorbox{{{hex_color}}}{{\\textsf{{{str(name[item])}}}}}"  # LaTeX equation
                            },
                            "annotations": {
                ...
                # Number column
                "Number": {
                    "number": float(num[item])  # Ensure number is a float
                },
                # Tag (select) column
                "Tag": {
                    "select": {
                        "name": tag[item]  # Ensure tag is a valid Notion Select value
                    }
                }
